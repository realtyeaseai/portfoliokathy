...
import { UserPlus, Share2, Globe, Phone, MessageCircle, QrCode, Play, Volume2, Maximize, Moon, Sun, Calendar, Calculator, Download, Star, Clock, MessageSquare, Video, Languages, Users, Award, CheckCircle, X } from "lucide-react";
...
export default function VirtualCard() {
  ...
  // Sample data
  const testimonials = [
    { name: "Sarah Johnson", role: "CEO, TechStart", rating: 5, text: "Outstanding VA services! They handled our web development needs perfectly." },
    { name: "Mike Chen", role: "Property Manager", rating: 5, text: "Reliable and professional. Helped streamline our property management processes." },
    { name: "Lisa Rodriguez", role: "Social Media Manager", rating: 5, text: "Creative and efficient social media management. Highly recommend!" }
  ];

  const services = [
    { name: "Web Development VA", price: "$25/hr", features: ["Frontend Development", "Backend Support", "Website Maintenance", "E-commerce Setup"] },
    { name: "Social Media VA", price: "$20/hr", features: ["Content Creation", "Posting Schedule", "Community Management", "Analytics Reports"] },
    { name: "Property Management VA", price: "$22/hr", features: ["Tenant Communication", "Listing Management", "Document Processing", "Maintenance Coordination"] },
    { name: "General Service VA", price: "$18/hr", features: ["Administrative Tasks", "Customer Support", "Data Entry", "Research & Analysis"] }
  ];

  const faqs = [
    { q: "What services do you provide?", a: "We offer Virtual Assistant services for Web Development, Social Media, Property Management, and general business support." },
    { q: "How quickly can you start?", a: "We can typically start within 24-48 hours after initial consultation and agreement." },
    { q: "What are your working hours?", a: "We work across multiple time zones and can accommodate most schedules. Available 7 days a week." },
    { q: "How do you ensure quality?", a: "All our VAs are thoroughly vetted, trained, and we provide regular progress updates and quality checks." }
  ];

  useEffect(() => {
    // Simulate card view increment
    const timer = setTimeout(() => {
      setCardViews(prev => prev + 1);
    }, 2000);
  ...
  return (
    <>
      <div className={`min-h-screen transition-all duration-500 ${darkMode ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900' : 'bg-gradient-to-br from-blue-50 via-white to-purple-50'} relative overflow-hidden`}>
        {/* Background Pattern */}
        ...
        <div className="relative z-10 flex justify-between items-center p-6">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2 bg-green-600/20 px-3 py-1 rounded-full">
              <div className={`w-2 h-2 rounded-full animate-pulse ${isOnline ? 'bg-green-500' : 'bg-gray-500'}`}></div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-800'}`}>DESIRED</span>
          </div>
        ...
        <div className="absolute top-20 right-6 z-10">
          <div className={`px-3 py-1 rounded-full ${darkMode ? 'bg-gray-800/50 text-gray-300' : 'bg-white/80 text-gray-700'} backdrop-blur-sm`}>
            <div className="flex items-center space-x-2">
              <Users className="w-3 h-3" />
              <span className="text-xs font-medium">{cardViews.toLocaleString()} views</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center min-h-[calc(100vh-120px)] px-4">
          ...
          <div className="max-w-2xl w-full mb-8 animate-fade-in">
            <div className={`relative rounded-2xl overflow-hidden shadow-2xl transform hover:scale-105 transition-all duration-500 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              {/* Video Container */}
              <div className={`aspect-video flex items-center justify-center relative group ${darkMode ? 'bg-gradient-to-br from-gray-700 to-gray-800' : 'bg-gradient-to-br from-blue-100 to-purple-100'}`}>
                {/* Clean gradient background */}
                <div className={`absolute inset-0 ${darkMode ? 'bg-gradient-to-br from-red-900/20 via-gray-800 to-amber-900/20' : 'bg-gradient-to-br from-blue-500/20 via-purple-100 to-pink-500/20'}`}></div>
                ...
                <div className="absolute top-4 right-4">
                  ...
                  >
                    <Volume2 className={`w-4 h-4 text-white ${isAudioPlaying ? 'animate-pulse' : ''}`} />
                  </button>
                </div>
                
                {/* Subtle overlay text */}
                <div className="absolute bottom-4 left-4 right-4 text-center">
                  <p className={`text-sm font-medium ${darkMode ? 'text-white/80' : 'text-gray-700/80'}`}>Professional Introduction Video</p>
                </div>
              </div>

              {/* Video Controls Bar */}
              <div className={`px-4 py-3 flex items-center justify-between ${darkMode ? 'bg-gray-900/90' : 'bg-gray-100/90'} backdrop-blur-sm`}>
                <div className="flex items-center space-x-3">
                  <div className="flex items-center space-x-2">
                    <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div>
                    <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Professional Showcase</span>
                  </div>
                </div>
                <div className="flex items-center space-x-3">
                  <button className={`transition-colors duration-300 ${darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-800'}`}>
          ...
          <div className="text-center mb-8 animate-slide-up">
            <h1 className={`text-3xl md:text-4xl font-bold mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>John Anderson</h1>
            <p className="text-red-400 font-medium text-lg mb-1">Founder & CEO</p>
            <p className={`mb-4 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Digital Innovations Inc.</p>
            
            {/* Service Description */}
            <div className={`backdrop-blur-sm rounded-xl p-6 max-w-2xl mx-auto border transform hover:scale-105 transition-all duration-300 ${darkMode ? 'bg-gray-800/30 border-gray-700/50' : 'bg-white/80 border-gray-200/50'}`}>
              <p className={`text-lg leading-relaxed ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                <span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>We provide Virtual Assistant</span> for any work you require - 
          ...
          <div className="grid grid-cols-4 gap-4 mb-8 animate-scale-in">
            ...
            >
              ...
              <span className={`text-xs font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Website</span>
            </button>
            ...
            >
              ...
              <span className={`text-xs font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Call Now</span>
            </button>
            ...
            >
              ...
              <span className={`text-xs font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>WhatsApp</span>
            </button>
            ...
            >
              ...
              <span className={`text-xs font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>QR Code</span>
            </button>
          </div>
      ...
      {showQRModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className={`rounded-2xl p-8 max-w-sm w-full text-center transform scale-100 transition-transform duration-300 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="mb-6">
              <h3 className={`text-2xl font-bold mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>QR Code</h3>
              <p className={`${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Scan to add to contacts</p>
            </div>

            <div className={`w-48 h-48 rounded-lg mx-auto mb-6 flex items-center justify-center ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
              <div className={`w-40 h-40 rounded-lg flex items-center justify-center ${darkMode ? 'bg-white opacity-20' : 'bg-black opacity-10'}`}>
                <QrCode className={`w-16 h-16 ${darkMode ? 'text-gray-300' : 'text-gray-400'}`} />
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className={`rounded-2xl p-8 max-w-md w-full ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-6">
              <h3 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Book a Meeting</h3>
              <button onClick={() => setShowCalendar(false)} className={`${darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-800'}`}>
                <X className="w-6 h-6" />
              </button>
            </div>

            <div className="space-y-4">
              <div className={`p-4 rounded-lg border-2 border-dashed cursor-pointer transition-colors ${darkMode ? 'border-gray-600 hover:border-blue-500 bg-gray-700/50' : 'border-gray-300 hover:border-blue-500 bg-gray-50'}`}>
                <div className="flex items-center space-x-3">
                  <Calendar className="w-5 h-5 text-blue-500" />
                  <div>
                    <p className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>Free Consultation</p>
                    <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>30 minutes • Free</p>
                  </div>
                </div>
              </div>

              <div className={`p-4 rounded-lg border-2 border-dashed cursor-pointer transition-colors ${darkMode ? 'border-gray-600 hover:border-green-500 bg-gray-700/50' : 'border-gray-300 hover:border-green-500 bg-gray-50'}`}>
                <div className="flex items-center space-x-3">
                  <Clock className="w-5 h-5 text-green-500" />
                  <div>
                    <p className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>Project Discussion</p>
                    <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>60 minutes • $50</p>
                  </div>
                </div>
              </div>
      ...
      {showCalculator && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className={`rounded-2xl p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-6">
              <h3 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Price Calculator</h3>
              <button onClick={() => setShowCalculator(false)} className={`${darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-800'}`}>
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="space-y-6">
              <div>
                <label className={`block text-sm font-medium mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Service Type</label>
              ...
              <div>
                <label className={`block text-sm font-medium mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Hours per Week</label>
                <input type="number" placeholder="20" className={`w-full p-3 rounded-lg border ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'}`} />
              </div>
              ...
              <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                <div className="flex justify-between items-center">
                  <span className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>Estimated Monthly Cost:</span>
                  <span className="text-2xl font-bold text-green-500">$2,000</span>
                </div>
              </div>
      ...
      {showTestimonials && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className={`rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-6">
              <h3 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Client Reviews</h3>
              <button onClick={() => setShowTestimonials(false)} className={`${darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-800'}`}>
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="space-y-6">
              {testimonials.map((testimonial, index) => (
                <div key={index} className={`p-6 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                  <div className="flex items-center mb-4">
                    ...
                    <div>
                      <p className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>{testimonial.name}</p>
                      <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{testimonial.role}</p>
                    </div>
                  </div>
                  <p className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{testimonial.text}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {showServices && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className={`rounded-2xl p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-6">
              <h3 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Our Services</h3>
              <button onClick={() => setShowServices(false)} className={`${darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-800'}`}>
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {services.map((service, index) => (
                <div key={index} className={`p-6 rounded-lg border-2 ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <h4 className={`text-lg font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>{service.name}</h4>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className={`rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowFAQ(false)} className={`${darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-800'}`}>
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              {faqs.map((faq, index) => (
                <div key={index} className={`p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                  <h4 className={`font-semibold mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{faq.q}</h4>
